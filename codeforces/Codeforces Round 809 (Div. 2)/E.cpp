#include <bits/stdc++.h>
using namespace std;
int now,tiempo,n,m,q,a,b;
vector<int> tin,pad,tam,val,ver,alt;
vector<vector<int>> graph,bj;
...
int main(){
    ...
    while(t--){
        ...
        tin.assign(2*n-1,0);
        graph.assign(2*n-1,{});
        bj.assign(2*n-1,vector<int>(20));
        now=n;
        for(int i=0;i<2*n-1;i++)tam[pad[i]=i]=1;
        for(int i=0;i<m;i++){
            cin >> a >> b;
            unite(--a,--b,i);
        }
